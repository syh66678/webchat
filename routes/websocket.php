<?php
    ...
    */
    ...
    WebsocketProxy::on('connect', function (WebSocket $websocket, Request $request) {
        ...
        //$websocket->emit('connect', '欢迎访问聊天室welcomen!');
    ...
    WebsocketProxy::on('login', function (WebSocket $websocket, $data) {
        if (!empty($data['api_token']) && ($user = User::where('api_token', $data['api_token'])->first())) {
            ...
            Log::info($user->name . '登陆成功啦！');
            $websocket->toUser($user)->emit('count', $rooms);
        } else {
            $websocket->emit('login', '登录后才能进入聊天室login field!');
        }
    });
    ...
    WebsocketProxy::on('room', function (WebSocket $websocket, $data) {
        if ($userId = $websocket->getUserId()) {
            ...
            if (empty($data['roomid'])) {
                // 打印日志
                Log::info($user->name . '进入房间失败：' . $roomId.'请传入房间id');
                return;
            }
            $roomId = $data['roomid'];
            // 重置用户与fd关联
            ...
            if (empty($count)) {
                // 打印日志
                Log::info($user->name . '进入房间失败：' . $roomId.'未查询到房间');
                return;
            }
            $count->count = 0;
            $count->save();
